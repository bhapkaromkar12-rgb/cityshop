...
const fs = require('fs');
const axios = require('axios'); // Nodemailer ki jagah HTTP API call ke liye
...
db.connect((err) => {
    ...
    // Automatic Table Creation Script (With Phone Column)
    const sql = `
...
// --- ROUTE: SEND OTP (PURE HTTP API ROUTE - NO NODEMAILER) ---
app.post('/send-otp', async (req, res) => {
    ...
    try {
        // Direct Web API call (Koi SMTP connection timeout ka darr nahi)
        const response = await axios.get('https://api.elasticemail.com/v2/email/send', {
            params: {
                apikey: process.env.EMAIL_API_KEY, // Aapki ElasticEmail API Key
                subject: 'City Shop Agro - Registration OTP Verification',
                from: process.env.EMAIL_FROM,      // Aapki verified sender email ID
                to: email,
                bodyHtml: emailHtml,
        ...
        if (response.data.success === true || response.data.map) { 
            res.json({ success: true, message: "OTP sent successfully to your email!" });
        } else {
            console.error("API Error Response:", response.data);
            res.status(500).json({ success: false, message: "API Response Fail!" });
        }
    } catch (err) {
        console.error("Email API Connection Error:", err.message);
...
app.put('/update-product/:id', upload.single('image'), (req, res) => {
    ...
    if (!req.body || Object.keys(req.body).length === 0) return res.status(400).send("Empty form!");
